...
// Página PÚBLICA de verificação de autenticidade de declarações (Fase B.2, G6).
// Rota única para todos os tipos de declaração emitidos pelo sistema — hoje só
// proficiência, mas o layout já lê genericamente o que vier em `dados`.
export default function VerificarDeclaracao() {
  ...
  useEffect(() => {
    document.title = 'Verificação de Declaração | PRPG UFRPE';
  ...
  useEffect(() => {
    ...
    apiFetch(`/api/declaracoes/${encodeURIComponent(codigo)}`, { auth: false })
      .then(async (r) => {
        if (!ativo) return;
        if (r.ok) {
          ...
          {estado === 'invalido' && (
            <div className="bg-white rounded-lg border border-red-200 p-8 text-center">
              ...
              <p className="text-gray-600 mt-3">
                O código informado não corresponde a uma declaração válida emitida pela PRPG/UFRPE.
                Confira se digitou o código corretamente ou escaneie novamente o QR code impresso
                no documento.
              </p>
