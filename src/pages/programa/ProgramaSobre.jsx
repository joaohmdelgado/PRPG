...
// Aba fixa do microsite: identidade do programa, o texto da página fixa
// "Sobre" (editável no admin — pages.chave='sobre'; endereço e vínculo com o
// programa travados, ver pagesController.js) e o histórico de coordenação.
// Outras páginas institucionais (Histórico, Objetivos…) têm endereço próprio
// (/<programaSlug>/<pageSlug> — ver ProgramaPagina.jsx), não ficam agregadas
// aqui.
export default function ProgramaSobre() {
  ...
  const sobre = programa.pagina_sobre;
  const temTexto = !!sobre?.body?.value?.trim();
          ...
          <>
            {temTexto && (
              <section className="bg-white rounded-2xl shadow-sm border border-gray-100 p-7 md:p-10">
                <SafeHtml
                  className="text-gray-700 leading-relaxed html-content prose prose-slate max-w-none"
                  html={sobre.body.value}
                />
              </section>
            )}
