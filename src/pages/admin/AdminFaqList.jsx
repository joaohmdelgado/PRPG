...
const AdminFaqList = () => {
  ...
  const fetchFaqs = async () => {
    try {
      const response = await apiFetch(withProgramaScope('/api/faq'));
      if (response.ok) {
  ...
  const handleDelete = async (id) => {
    if (await confirm('Tem certeza que deseja excluir esta pergunta frequente?')) {
      try {
        const response = await apiFetch(`/api/faq/${id}`, { method: 'DELETE' });
  ...
  const handleBulkDelete = async () => {
    ...
    setDeleting(true);
    await bulkDelete('/api/faq', selectedIds, { onUnauthorized: () => navigate('/admin/login') });
    setDeleting(false);
    clear();
    fetchFaqs();
  };
