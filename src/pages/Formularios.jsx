...
export default function Formularios() {
    ...
    const fetchFormularios = async () => {
      try {
        ...
        data.forEach(item => {
          ...
          const sid = sectionId || 'apoio-financeiro';
  ...
  const filteredData = formulariosData.map(section => {
          ...
          <nav className="flex text-white/60 text-sm mb-4" aria-label="Breadcrumb">
            <ol className="inline-flex items-center space-x-1 md:space-x-3">
              ...
              <li aria-current="page">
                <div className="flex items-center">
                  <i className="fa-solid fa-chevron-right text-[10px] mx-2 opacity-50"></i>
                  <span className="text-ufrpe-yellow font-medium">Formulários</span>
                </div>
              </li>
            </ol>
          </nav>
          <h1 className="text-4xl md:text-5xl font-heading font-extrabold">Formulários</h1>
          <p className="text-white/70 mt-4 text-lg">
            Consulte e baixe os formulários necessários para os processos da Pró-Reitoria de Pós-Graduação.
          </p>
        </div>
      </div>
              ...
              {filteredData.length === 0 ? (
                ...
                filteredData.map(section => (
                  <section key={section.id} id={section.id} className="scroll-mt-28 space-y-6">
                    <h2 className="font-heading font-bold text-2xl md:text-3xl text-ufrpe-blue pb-4 border-b-2 border-ufrpe-yellow">
                      {section.title}
                    </h2>
                    ...
                    <div className="space-y-4">
                      {section.categories.map((cat, catIdx) => (
                        <details
                          ...
                          open={searchTerm !== ''} // Auto open if there is a search term
                        >
